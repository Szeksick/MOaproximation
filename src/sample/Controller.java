...
private ArrayList<ArrayList<Double>> matrixS, matrixT;
private Double h;
    ...
    public void calculate(ActionEvent e) {
        ...
        if (selectedFile == null) {
            ...
            try {
                ...
                matrixS = new ArrayList<ArrayList<Double>>();
                for(int i = 0; i<tabx.size()-1; i++) {
                    matrixS.add(new ArrayList<Double>());
                    if(i==0){
                        for (int j = 1; j <tabx.size(); j++) {
                            matrixS.get(i).add(taby.get(j)-taby.get(j-1));
                        }
                    }else {
                        for (int j = 1; j <tabx.size() - i; j++) {
                            matrixS.get(i).add(matrixS.get(i-1).get(j)-matrixS.get(i-1).get(j-1));
                        }
                    }
                }
                matrixT = new ArrayList<ArrayList<Double>>();
                for(int i = 0; i<tabx.size()-1; i++) {
                    matrixT.add(new ArrayList<Double>());
                    if(i==0){
                        for (int j = 1; j <tabx.size(); j++) {
                            matrixT.get(i).add(taby.get(j)-taby.get(j-1));
                        }
                    }else {
                        for (int j = 1; j <tabx.size() - i; j++) {
                            matrixT.get(i).add(matrixT.get(i-1).get(j)-matrixT.get(i-1).get(j-1));
                        }
                    }
                }
